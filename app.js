...
app.get("/dashboard/total/:id", (req, res) => {
  try {
    ...
    pool.getConnection((err, connection) => {
      ...
      connection.query(
        "SELECT count(*) as total FROM bills WHERE employee_id = ?",
        [req.params.id],
        (err, rows) => {
          connection.release() //return the connection to the pool
...
app.get("/dashboard/month/:id/:month", (req, res) => {
  try {
    ...
    pool.getConnection((err, connection) => {
      ...
      connection.query(
        "SELECT id FROM bills WHERE employee_id = ? AND MONTH(bill_date) = ?",
...
app.get("/dashboard/today/:id", (req, res) => {
  try {
    ...
    pool.getConnection((err, connection) => {
      ...
      connection.query(
        "SELECT id FROM bills WHERE employee_id = ? AND DATE(bill_date) = CURDATE()",
        [req.params.id],
        (err, rows) => {
          connection.release() //return the connection to the pool
...
app.put("/billpayment/:id", (req, res) => {
  try {
    ...
    pool.getConnection((err, connection) => {
      ...
      connection.query(
        "UPDATE bills SET payment_status = ?, paid_amount=?, discount_amount =?, admin_message=? WHERE id = ?",
        [
          req.body.pay_status,
...
app.post("/getbillsrange/:userid", (req, res) => {
  try {
    ...
    pool.getConnection((err, connection) => {
      ...
      connection.query(
        "SELECT * FROM bills WHERE employee_id = ? AND ( bill_date >= ? AND bill_date <= ?)",
        [req.params.userid, req.body.date1, req.body.date2],
        (err, rows) => {
          connection.release() //return the connection to the pool
      ...
      if (req.body.search != "") {
        connection.query(
          ...
          (err, rowdata) => {
            ...
            if (
              rowdata[0].employee_name == "HO" ||
              rowdata[0].employee_name == "BO"
            ) {
              connection.query(
                `SELECT b.*,e.branch_id,e.employee_name FROM bills b
                                    JOIN employee e ON b.employee_id = e.id
                                    WHERE b.invoice_id = ? AND e.branch_id = ?`,
                [req.body.search, rowdata[0].branch_id],
                (err, rows) => {
                  connection.release() //return the connection to the pool
              ...
              connection.query(
                `SELECT * FROM bills WHERE (customer_phoneno = ? OR invoice_id = ? OR customer_name= ?)  AND employee_id = ?`,
                [
                  req.body.search,
                  req.body.search,
                  req.body.search,
...
app.get("/getsersign/:id", (req, res) => {
  try {
    ...
    pool.getConnection((err, connection) => {
      ...
      connection.query(
        "SELECT s_sign FROM bills WHERE id = ?",
        [req.params.id],
        (err, rows) => {
          connection.release() //return the connection to the pool
...
app.get("/getcussign/:id", (req, res) => {
  try {
    ...
    pool.getConnection((err, connection) => {
      ...
      connection.query(
        "SELECT c_sign FROM bills WHERE id = ?",
        [req.params.id],
        (err, rows) => {
          connection.release() //return the connection to the pool
...
app.get("/mycreditbills/:userid", (req, res) => {
  try {
    ...
    pool.getConnection((err, connection) => {
      ...
      connection.query(
        "SELECT * FROM bills WHERE employee_id = ? AND payment_status = ? AND payment_method= ?",
        [req.params.userid, payment_status, payment_method],
        (err, rows) => {
          connection.release() //return the connection to the pool
...
app.get("/getmanagers/:adminid", (req, res) => {
  try {
    ...
    pool.getConnection((err, connection) => {
      ...
      connection.query(
        `SELECT admin_table.*, branch.branch_name FROM admin_table JOIN branch ON branch.id = admin_table.branch_id AND admin_table.id <> ?`,
        [req.params.adminid],
        (err, rows) => {
          connection.release() //return the connection to the pool
  ...
  try {
    ...
    pool.getConnection((err, connection) => {
      if (!err) {
        ...
        connection.query(
          "UPDATE branch SET branch_name = ?  WHERE id = ?",
        ...
        connection.query(
          ...
          (err, bill) => {
            if (!err) {
              ...
              connection.query(
                "DELETE FROM machine WHERE bill_id = ?",
                [req.params.billid],
                (err, rows) => {
                  if (!err) {
                    connection.query(
  ...
  try {
    pool.getConnection((err, connection) => {
      if (!err) {
        ...
        connection.query(
          "UPDATE admin_table SET name = ?, username= ?, password=?  WHERE id = ?",
...
app.delete("/delmanager/:managerid", (req, res) => {
  try {
    pool.getConnection((err, connection) => {
      if (!err) {
        connection.query(
          "DELETE FROM admin_table WHERE id = ?",
...
app.get("/totalbillscount/:branchid", (req, res) => {
  try {
    pool.getConnection((err, connection) => {
      if (!err) {
        connection.query(
          `SELECT count(*) as total_bills FROM bills JOIN employee ON employee.id = bills.employee_id 
...
app.get("/monthbillscount/:branchid/:month", (req, res) => {
  try {
    pool.getConnection((err, connection) => {
      if (!err) {
        connection.query(
          `SELECT count(*) as month_bills FROM bills JOIN employee 
                            ON bills.employee_id = employee.id AND employee.branch_id = ? AND MONTH(bill_date) = ?`,
          [req.params.branchid, req.params.month],
          (err, rows) => {
            connection.release() //return the connection to the pool
  ...
  try {
    pool.getConnection((err, connection) => {
      if (!err) {
        ...
        connection.query(
          `SELECT count(*) as total_bills FROM bills JOIN employee ON employee.id = bills.employee_id 
                            AND employee.branch_id = ? AND bills.payment_status = ?`,
          [req.params.branchid, pend],
          (err, rows) => {
            connection.release() //return the connection to the pool
...
app.get("/todaybillscount/:branchid", (req, res) => {
  try {
    pool.getConnection((err, connection) => {
      ...
      connection.query(
        `SELECT count(*) as total FROM bills JOIN employee 
                    ON bills.employee_id = employee.id AND employee.branch_id = ? AND DATE(bill_date) = CURDATE()`,
        [req.params.branchid],
        (err, rows) => {
          connection.release() //return the connection to the pool
...
app.put("/updateemp", (req, res) => {
  try {
    pool.getConnection((err, connection) => {
      ...
      connection.query(
        "UPDATE employee SET employee_name = ?, username= ?, password=?  WHERE id = ?",
...
app.post("/uploadimgs3", (req, res) => {
  try {
    ...
    pool.getConnection(async (err, connection) => {
      ...
      buf1 = Buffer.from(
        req.body.s_sign.replace(/^data:image\/\w+;base64,/, ""),
        "base64"
      )
      const result1 = await uploadBase64(buf1, s_sign_name)
      if (result1 === 0) {
        buf2 = Buffer.from(
          req.body.c_sign.replace(/^data:image\/\w+;base64,/, ""),
          "base64"
        )
        const result2 = await uploadBase64(buf2, c_sign_name)
        if (result2 === 0) {
          connection.query(
            "UPDATE bills SET s_sign = ?, c_sign = ? WHERE id = ?",
            [s_sign_name, c_sign_name, req.body.id],
            (err, rows) => {
              connection.release() //return the connection to the pool
    ...
    pool.getConnection((err, connection) => {
      if (!err) {
        ...
        if (req.body.month != "ALL") {
          connection.query(
            `SELECT e.id as id,e.username,e.employee_name, SUM(b.s_charge) AS total_amount,
            ...
            [req.params.id, req.body.year, req.body.month],
            (err, rows) => {
              connection.release() //return the connection to the pool
